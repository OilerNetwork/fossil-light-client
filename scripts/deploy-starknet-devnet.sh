...
update_env_var() {
    ...
    if grep -q "^$var_name=" "$env_file"; then
        ...
        cat "$env_file" | sed "s|^$var_name=.*|$var_name=$var_value|" > "$tmp_file"
...
FOSSILSTORE_HASH=$(starkli declare ./target/dev/fossil_store_Store.contract_class.json --account config/devnet-account --private-key $DEVNET_ACCOUNT_PK --rpc $STARKNET_RPC_URL | grep -o '0x[a-fA-F0-9]\{64\}' | head -1)
echo -e "${GREEN}Class hash declared: ${BOLD}$FOSSILSTORE_HASH${NC}"
echo

echo -e "${YELLOW}Deploying Fossil Store contract...${NC}"
FOSSILSTORE_ADDRESS=$(starkli deploy $FOSSILSTORE_HASH $DEVNET_ACCOUNT_ADDR --account config/devnet-account --private-key $DEVNET_ACCOUNT_PK --rpc $STARKNET_RPC_URL -w | grep -o '0x[a-fA-F0-9]\{64\}' | head -1)
...
L1MESSAGEPROXY_HASH=$(starkli declare ./target/dev/l1_message_proxy_L1MessageProxy.contract_class.json --account config/devnet-account --private-key $DEVNET_ACCOUNT_PK --rpc $STARKNET_RPC_URL | grep -o '0x[a-fA-F0-9]\{64\}' | head -1)
echo -e "${GREEN}Class hash declared: ${BOLD}$L1MESSAGEPROXY_HASH${NC}"
echo

echo -e "${YELLOW}Deploying Fossil L1MessageProxy contract...${NC}"
L1MESSAGEPROXY_ADDRESS=$(starkli deploy $L1MESSAGEPROXY_HASH $L1_MESSAGE_SENDER $FOSSILSTORE_ADDRESS $DEVNET_ACCOUNT_ADDR --account config/devnet-account --private-key $DEVNET_ACCOUNT_PK --rpc $STARKNET_RPC_URL -w | grep -o '0x[a-fA-F0-9]\{64\}' | head -1)
...
ECIP_HASH=$(starkli declare ./target/dev/verifier_UniversalECIP.contract_class.json --account config/devnet-account --private-key $DEVNET_ACCOUNT_PK --rpc $STARKNET_RPC_URL | grep -o '0x[a-fA-F0-9]\{64\}' | head -1)
...
VERIFIER_HASH=$(starkli declare ./target/dev/verifier_Risc0Groth16VerifierBN254.contract_class.json --account config/devnet-account --private-key $DEVNET_ACCOUNT_PK --rpc $STARKNET_RPC_URL | grep -o '0x[a-fA-F0-9]\{64\}' | head -1)
echo -e "${GREEN}Class hash declared: ${BOLD}$VERIFIER_HASH${NC}"
echo

echo -e "${YELLOW}Deploying Groth16 Verifier contract...${NC}"
VERIFIER_ADDRESS=$(starkli deploy $VERIFIER_HASH $ECIP_HASH --account config/devnet-account --private-key $DEVNET_ACCOUNT_PK --rpc $STARKNET_RPC_URL | grep -o '0x[a-fA-F0-9]\{64\}' | head -1)
echo -e "${GREEN}Contract deployed at: ${BOLD}$VERIFIER_ADDRESS${NC}"
echo

echo -e "${YELLOW}Declaring Fossil Verifier contract...${NC}"
FOSSIL_VERIFIER_HASH=$(starkli declare ./target/dev/verifier_FossilVerifier.contract_class.json --account config/devnet-account --private-key $DEVNET_ACCOUNT_PK --rpc $STARKNET_RPC_URL | grep -o '0x[a-fA-F0-9]\{64\}' | head -1)
echo -e "${GREEN}Class hash declared: ${BOLD}$FOSSIL_VERIFIER_HASH${NC}"
echo

echo -e "${YELLOW}Deploying Fossil Verifier contract...${NC}"
FOSSIL_VERIFIER_ADDRESS=$(starkli deploy $FOSSIL_VERIFIER_HASH $VERIFIER_ADDRESS $FOSSILSTORE_ADDRESS $DEVNET_ACCOUNT_ADDR --account config/devnet-account --private-key $DEVNET_ACCOUNT_PK --rpc $STARKNET_RPC_URL -w | grep -o '0x[a-fA-F0-9]\{64\}' | head -1)
...
if [ "$ENV_TYPE" = "local" ] || [ "$ENV_TYPE" = "docker" ]; then
    ...
    starkli invoke $FOSSILSTORE_ADDRESS initialize $FOSSIL_VERIFIER_ADDRESS $L1MESSAGEPROXY_ADDRESS $UPDATE_INTERVAL --account config/devnet-account --private-key $DEVNET_ACCOUNT_PK --rpc $STARKNET_RPC_URL -w
