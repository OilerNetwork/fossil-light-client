...
# Run clippy on crates without RISC0 dependencies
cargo clippy \
    --no-deps \
    -p common \
    -p ethereum \
...
echo "Running clippy on RISC0-dependent crates (with build skip)..."
# Run clippy on crates with RISC0 dependencies by skipping the RISC0 build process
# Note: 'methods' crate is still excluded as it contains guest code that cannot be linted
