...
impl ProofService {
    ...
    pub const fn with_config(config: PublisherConfig) -> Self {
        Self { config }
    }
    ...
    pub async fn get_single_block_hash_proof(
        ...
        let (store_manager, mmr, pool) = mmr_utils::initialize_mmr(&batch_file_name)
            .await
            .map_err(|e| PublisherError::mmr_operation(format!("Failed to initialize MMR: {e}")))?;
        ...
        let peaks = mmr
            ...
            .await
            .map_err(|e| PublisherError::mmr_operation(format!("Failed to retrieve peaks: {e}")))?;
        ...
        builder
            ...
            .map_err(|e| {
                PublisherError::proof_generation(format!(
                    "Failed to update MMR with new headers: {e}"
                ))
            })
    }
    ...
    async fn fetch_and_validate_mmr_db(
        ...
        let batch_file_name = common::get_or_create_db_path(&format!("batch_{batch_index}.db"))
            .map_err(|e| PublisherError::configuration(format!("Failed to create DB path: {e}")))?;
    ...
    async fn validate_mmr_state(
        ...
        let mmr_root = starknet_handler::u256_from_hex(&mmr_root_hex)
            .map_err(|e| PublisherError::serialization(format!("Failed to parse MMR root: {e}")))?;
    ...
    async fn get_batch_info_for_block_hash(
        ...
        let header = db_connection
            ...
            .await
            .map_err(|e| PublisherError::validation(format!("Failed to get block header: {e}")))?;
