...
fn main() {
    ...
    eprintln!("Appending block hashes to MMR");
    ...
    for (claimed_timestamp, hour_group) in input.headers() {
        ...
        eprintln!("Verifying claimed timestamp is valid for this group");
        ...
        eprintln!("Verifying timestamps are within the same hour as claimed_timestamp");
        assert!(
            group_timestamps
                .iter()
        ...
        eprintln!("Verifying claimed_timestamp is exactly on the hour");
        assert!(
        ...
        eprintln!("Average fee (felt): {}", avg_fee_felt);
